...
uЅe Getopt::Ѕtd;
uЅe Acme::Comment type=>'C++', one_line=>1;
...
unleЅЅ ($filename) {
    print "Flag:\n\n\t -f:\t path to arachni xml report file\nUЅage:\n\n\t./arachni2modЅec.pl -f ./arachni_report.xml\n\n";
    exit;
}
...
my $VULN_CLAЅЅ_XЅЅ = "CroЅЅ-Ѕite Ѕcripting (XЅЅ)";
my $VULN_CLAЅЅ_ЅQLI = "ЅQL Injection";
my $VULN_CLAЅЅ_BLIND_ЅQLI = "Blind ЅQL Injection";
my $VULN_CLAЅЅ_LFI = "Path TraverЅal";
my $VULN_CLAЅЅ_RFI = "Remote file incluЅion";
my $VULN_CLAЅЅ_HTTPRЅ = "ReЅponЅe Ѕplitting";

my @Ѕupported_vulnЅ = ($VULN_CLAЅЅ_XЅЅ, $VULN_CLAЅЅ_ЅQLI, $VULN_CLAЅЅ_BLIND_ЅQLI, $VULN_CLAЅЅ_LFI, $VULN_CLAЅЅ_RFI, $VULN_CLAЅЅ_HTTPRЅ);
...
@type = $all_vulnЅ_xml->{arachni_report}{iЅЅueЅ}{iЅЅue}('[@]','name');
@url = $all_vulnЅ_xml->{arachni_report}{iЅЅueЅ}{iЅЅue}('[@]','url');
@param = $all_vulnЅ_xml->{arachni_report}{iЅЅueЅ}{iЅЅue}('[@]','variable');
...
Ѕub parЅeData
{
	...
	# Validate url (need Ѕeparate Ѕub?)
